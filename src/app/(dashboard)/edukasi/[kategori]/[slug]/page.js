  ...
  return (
    <div className="space-y-6 max-w-7xl w-full mx-auto">
      ...
      <section className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm space-y-6">
        ...
        <div className="space-y-3 pt-2">
          {educationContent[activeTab] && educationContent[activeTab].map((item, idx) => (
            <div 
              key={idx}
              className="w-full bg-white border border-emerald-100/40 rounded-2xl overflow-hidden transition-all duration-200 shadow-sm"
            >
              <button 
                type="button"
                suppressHydrationWarning
                onClick={() => setOpenIdx(openIdx === idx ? null : idx)}
                className="w-full flex items-center justify-between p-4 cursor-pointer hover:bg-emerald-50/30 transition-all duration-200 group text-left"
              >
                ...
                <ChevronRight 
                  size={16} 
                  className={`text-emerald-500 group-hover:text-emerald-700 transition-transform duration-200 ${openIdx === idx ? 'rotate-90' : ''}`} 
                />
              </button>

              {openIdx === idx && (
                <div className="px-5 pb-5 pt-1 space-y-3 text-slate-600 text-xs border-t border-dashed border-emerald-100/60 bg-emerald-50/10">
      ...
      <button 
        type="button"
        suppressHydrationWarning
        className="fixed bottom-6 right-6 bg-emerald-500 text-white pl-4 pr-6 py-3 rounded-full shadow-lg flex items-center gap-4 hover:bg-emerald-600 hover:scale-105 transition-all duration-200 font-bold text-xs border border-emerald-400 z-50 group"
      >
