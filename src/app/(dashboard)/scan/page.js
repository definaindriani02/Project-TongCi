...
export default function KlasifikasiAI() {
  ...
  const categories = [
    { name: "Organik", desc: "Tempat sampah hijau", color: "bg-emerald-500", text: "text-emerald-700", border: "border-emerald-200", icon: "🥬" },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* SCANNER CONTAINER */}
      <section className="bg-white rounded-3xl p-6 md:p-8 border border-emerald-50 shadow-sm min-h-[420px] flex flex-col justify-between relative overflow-hidden">
        ...
        {successMsg && (
          <div className="my-4 bg-emerald-50 border border-emerald-100 p-3.5 rounded-2xl flex items-start gap-2.5 text-emerald-700 text-xs font-semibold">
        ...
        {scanState === "idle" && (
          <div className="flex flex-col items-center justify-center my-auto py-8 text-center">
            <div className="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center border border-emerald-100 text-4xl mb-4 relative">
              🗑️
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xs">
              <button
                ...
                className="flex-1 bg-emerald-500 hover:bg-emerald-600 active:scale-95 transition-all text-white font-bold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-1.5 shadow-md shadow-emerald-100 cursor-pointer"
              >
                <Upload size={16} /> Upload Foto
              </button>
              <button
                onClick={startCamera}
                className="flex-1 bg-white hover:bg-slate-50 border border-emerald-500 hover:border-emerald-600 text-emerald-600 hover:text-emerald-700 font-bold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-1.5 transition-all active:scale-95 cursor-pointer"
              >
          ...
          <div className="flex flex-col items-center justify-center my-auto py-4 relative w-full max-w-md mx-auto">
            ...
            <div className="flex gap-4 mt-6">
              <button
                ...
                className="bg-emerald-500 hover:bg-emerald-600 active:scale-95 transition-all text-white font-bold text-xs py-2.5 px-6 rounded-xl flex items-center gap-1.5 shadow-md shadow-emerald-100 cursor-pointer"
              >
        ...
        {scanState === "result" && scanResult && (
          <div className="flex flex-col my-auto py-4 space-y-6">
            ...
            <div className={`p-5 rounded-2xl border flex items-start gap-4 ${
              scanResult.category === "Organik" ? "bg-emerald-50/50 border-emerald-100" :
              ...
              <div className="space-y-1">
                <h4 className="font-extrabold text-sm text-slate-800">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ml-2 inline-block ${
                    scanResult.category === "Organik" ? "bg-emerald-100 text-emerald-700" :
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              ...
              <button
                ...
                className="flex-1 bg-white hover:bg-slate-50 border border-emerald-500 text-emerald-600 hover:text-emerald-700 font-bold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-1.5 transition-all active:scale-95 disabled:bg-slate-50 disabled:text-slate-400 disabled:border-slate-200 cursor-pointer"
              >
                {saving ? (
                  <div className="w-4 h-4 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin"></div>
                ) : pointsSaved ? (
                  <>
                    <CheckCircle size={16} className="text-emerald-500" /> Tersimpan
                  </>
                ) : (
                  <>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {categories.map((cat, idx) => (
          ...
          >
            ...
            <p className="text-[10px] text-emerald-600 font-bold mt-1">{cat.desc}</p>
          </div>
        ))}
      </section>
